...
ffighters = r'E:\spacece\project 3\ffighters'
question_json ='E:\spacece\project 3\questions.json'
...
if os.path.isdir(folder_path):
    ...
    quiz_file_path = question_json  # Path to your JSON file
    quiz_questions = load_quiz_questions(quiz_file_path)

    # Create session states for the buttons and pagination
    ...
    for fighter in freedom_fighters:
        ...
        # Display the fighter's name and image
    ...
    with st.expander("Start Quiz"):
        ...
        # Get the questions to show based on the current page
        start_idx = st.session_state.page * 10
        end_idx = start_idx + 10
        questions_to_show = quiz_questions[start_idx:end_idx]

        for idx, question in enumerate(questions_to_show, start=start_idx + 1):
            st.subheader(f"{idx}. {question['question']}")

            # Display options
            answer = st.radio(f"Choose your answer for Q{idx}", options=question["options"], key=f"q{idx}")

            # Button for typed answer submission
            ...
            # Button for spoken answer submission
            if st.button(f"Speak Answer for Q{idx}", key=f"speak{idx}"):
                spoken_answer = capture_speech()
                if spoken_answer:
                    st.write(f"You said: {spoken_answer}")
                    if spoken_answer.lower() == question["answer"].lower():
                        st.success("Correct!")
                    else:
                        st.error(f"Wrong! The correct answer is: {question['answer']}")

        # Pagination buttons
        if len(quiz_questions) > end_idx:
            if st.button("Next"):
    ...
    st.error(f"The folder path '{folder_path}' does not exist. Please ensure the path is correct.")